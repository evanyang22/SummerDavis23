...
```{r}
library(NOISeq)
#write.table(dfJustinSerum1W,"./test.txt")
...
noiseq(inputEY, k = 0.5, norm = "rpkm", 
replicates = "no", 
factor="Time", conditions=c("Justin_PO_Serum","Justin_1w_Serum"), pnr = 0.2, nss = 5, v = 0.02, lc = 0)
```

```{r}
seqInfo_EY_JustinSerum1W <- subset(seqInfo_EY, Sheep %in% c("Justin"))[c(1,2),]

dfJustinSerum<-fullDF_filtered%>% select(contains("Justin"))
dfJustinSerum1W <- dfJustinSerum[,c(1,2)]
Operation<-factor(seqInfo_EY_JustinSerum1W$Operation, levels = c('Pre', 'Post'))

designEY_JustinSerum1W<-model.matrix(~ Operation)
...
vobjDreamEY <- voomWithDreamWeights(dfJustinSerum1W, form, seqInfo_EY_JustinSerum1W, BPPARAM = param)
fitmmEY <- dream(vobjDreamEY, form, seqInfo_EY_JustinSerum1W)
fitmmEY <- treat(fitmmEY, lfc = log2(1.2), trend = TRUE, robust = TRUE)

finalEY <- topTable(fitmmEY, coef = 2, sort.by = "logFC", number = Inf) %>%
     mutate(log10P = -log10(adj.P.Val), .after = adj.P.Val)
write.csv(finalEY, "./JustinSerum1W.csv")
